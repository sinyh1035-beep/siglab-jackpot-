...
# 비용/한도 관리
MAX_STOCKS_PER_RUN = 30  # 시총 상위 30개로 비용 절감  # 시총 상위 100개만
NEWS_PER_SOURCE = 4  # 네이버 4 + 구글 4 = 8건  # 각 출처에서 8건씩 (총 16건)
MAX_NEWS_FOR_CLAUDE = 8  # Claude 비용 절감  # Claude에 보낼 최대 뉴스 (비용 관리)
...
def merge_news(naver_items: list, google_items: list) -> list:
    """네이버 + 구글 뉴스 합치고 중복 제거 + 발행일 필터"""
    from datetime import datetime, timedelta
    all_news = naver_items + google_items
    seen_titles = set()
    unique_news = []

    # 최근 2일 이내 뉴스만 (오래된 뉴스 제외)
    now = datetime.now()
    ...
    for n in all_news:
        ...
        if pub_date_str:
            try:
                ...
                if pub_naive < cutoff:
                    continue  # 2일 이상 지난 뉴스 제외
            except (ValueError, TypeError):
                pass  # 파싱 실패 시 일단 통과 (안전 폴백)
