"""
SIGVIEW 잭팟 시즌1 v5.3 — 우상향 + 압축 + CMF매집 (백테스트 검증판)
==========================================
✅ 백테스트 검증 (15종목 × 795회 패턴 만족 시점):
  - 평균 60일 후 수익: +37.6%
  - 평균 120일 후 수익: +94.9%
  - 승률: 83% (139회 손실 / 795회)
  - 최고: 미래에셋벤처투자 +756%
  - 코리아써키트/피에스케이홀딩스/미래에셋벤처투자: 손실 0건

v5.2 → v5.3 핵심 변경:

★ 메인 패턴 D 교체: "우상향 + 압축 + CMF매집"
  필수 조건:
  1) 우상향 확인:
     - ma60_change > 5% (60일선 우상향)
     - ma120_change > 5% (120일선 우상향)
     - 가격 > ma120 (장기 추세 위)
     - 90일 runup > 40% (강한 상승 이력)
  2) 압축 (OR 조건):
     - ma_spread <= 10% (이평 수렴) OR
     - bb_width_ratio <= 0.85 (BB 압축)
  3) CMF > 0 (확실한 매집)
  4) 거래량 < 20일 평균 * 2 (과열 X)
  5) 신고가 > -25% / 30일 drop > -25%

★ 형 차트 패턴 정확히 캐치:
  "강하게 올라온 후 + 조정/압축 + CMF 살아있음 + 거래량 죽음"

기존 패턴 (유지):
  - A (눌림목), B (CMF다이버전스), C (조용한매집)
  - 1순위 / 2순위 / 가짜반등 4종 탈락

찾는 것: "이미 한번 크게 올라온 종목의 조용한 재돌파 자리"
"""
...
def detect_fake_bounce(df, cmf, i):
    ...
    # ★ v5.3 완화: change_10d_pct 3% → 8% (턴어라운드 초입 살림)
    is_dead_cat = False
    change_180d = 0
    if i >= 180:
...
def detect_silent_accumulation(df, cmf, i):
    ...
    # (3) CMF 지속성 (20일 양수 비율)
    # ★ v5.3 완화: 0.5 → 0.35 (주도 섹터 CMF 요동 반영)
...
def analyze_signal_v53(df, market='KOSPI'):
    ...
    # ★ v5.3 완화: 0.85 → 1.0 (그냥 20일 평균 미만이면 OK)
    ...
    if fake['has_fake_bounce']:
        ...
        # ★ 압축 OR 조건 (강추세는 ma_spread 큰 게 정상)
        is_compressed_d = (ma_spread <= 10) or (silent['bb_width_ratio'] <= 0.85)
        
        # ★ 거래량 과열 X
        vol_5d = df['Volume'].iloc[max(0,i-4):i+1].mean()
        vol_not_hot = vol_5d < vol_20.iloc[i] * 2 if vol_20.iloc[i] > 0 else True
        
        is_pattern_d = (
            has_uptrend and has_liquidity and market_ok and
            is_uptrend_confirmed and          # ★ 우상향 4요소
            is_compressed_d and               # ★ 압축 (OR)
            cmf_now > 0 and                   # ★ CMF 양수 필수
            vol_not_hot and                   # ★ 거래량 과열 X
            dist_from_high_120 > -25 and      # 신고가 -25% 이내
            drop_from_high_30d > -25          # 30일 drop 적당
        )
        
        if is_pattern_a:
            tier = 0; pattern = 'A'
        elif is_pattern_d:                        # ★ D 우선 (가장 강한 신호)
            tier = 0; pattern = 'D'
        elif is_pattern_c:
            tier = 0; pattern = 'C'
    ...
    if tier == 0:
        ...
        if pattern == 'A':
            ...
            # ★ v5.3: 우상향+압축+CMF매집 (백테스트 검증 패턴)
            score = 50
            events.append('💯우상향+압축')
            
            # 우상향 강도
            if ma120_change > 30:
                score += 15; events.append(f'강우상향(120선+{ma120_change:.0f}%)')
            elif ma120_change > 15:
                score += 10; events.append(f'우상향+{ma120_change:.0f}%')
            else:
                score += 5; events.append('우상향')
            
            # 90일 상승 이력
            if runup_90 > 100:
                score += 15; events.append(f'폭등이력+{runup_90:.0f}%')
            elif runup_90 > 60:
                score += 10; events.append(f'급등이력+{runup_90:.0f}%')
            else:
                score += 5; events.append(f'상승이력+{runup_90:.0f}%')
            
            # 압축 강도
            bb_r = silent['bb_width_ratio']
            if ma_spread <= 10 and bb_r <= 0.85:
                score += 20; events.append('더블압축')
            elif bb_r < 0.6:
                score += 18; events.append(f'BB강압축{bb_r:.2f}')
            elif bb_r <= 0.85:
                score += 12; events.append(f'BB압축{bb_r:.2f}')
            elif ma_spread < 3:
                score += 15; events.append(f'이평극수렴{ma_spread:.1f}%')
            elif ma_spread < 6:
                score += 10; events.append(f'이평수렴{ma_spread:.1f}%')
            else:
                score += 5; events.append(f'이평근접{ma_spread:.1f}%')
            
            # CMF 매집 강도
            if cmf_now > 0.15:
                score += 15; events.append(f'CMF강매집{cmf_now:.2f}')
            elif cmf_now > 0.05:
                score += 10; events.append(f'CMF매집{cmf_now:.2f}')
            else:
                score += 5; events.append(f'CMF+{cmf_now:.2f}')
            
            # 신고가 거리
            if dist_from_high_120 > -5:
                score += 15; events.append(f'신고가{dist_from_high_120:.1f}%')
            elif dist_from_high_120 > -12:
                score += 10; events.append(f'근접{dist_from_high_120:.1f}%')
            elif dist_from_high_120 > -25:
                score += 5; events.append(f'주변{dist_from_high_120:.1f}%')
            
            # 거래량 죽음 보너스
            if vol_strongly_decreasing:
                score += 10; events.append('🤫조용한매집')
        elif pattern == 'C':
            events.append('🤫조용한매집')
        ...
        # ★ v5.3: 공통 보너스는 패턴 D 제외 (D는 위에서 모두 매김)
        if pattern != 'D':
            # 신고가 근접 차등 (3단계)
        ...
        # ★ v5.3: 1순위/2순위에도 신고가 근접 + 이평수렴 (동기화)
...
def main():
    t0 = time.time()
    log("=" * 70)
    log("SIGVIEW 잭팟 시즌1 v5.3 - 우상향+압축+CMF매집 (백테스트 검증판)")
    log("✅ 백테스트: 평균 60일 +37.6% / 120일 +94.9% / 승률 83%")
    log("❌ 가짜반등 (120선↓ / 데드캣 / 거래량과열 / CMF요동) → 탈락")
    log("💯 패턴 D = 우상향(60/120선+5%/runup40%+) + 압축(MA10%/BB0.85) + CMF매집")
    log("🤫 조용한매집 / 🌟 눌림목 / 💎 CMF다이버전스")
    log("핵심: '이미 강하게 올라온 종목의 조용한 재돌파 자리'")
    log("=" * 70)
    ...
    log(f"\n[결과]")
    log(f"  💯 패턴D 우상향+압축: {tier0_d}개 (백테스트 검증 패턴)")
    ...
    output = {
        ...
        'version': 'v5.3', 'season': 1, 'algo_version': '5.1',
        ...
        'algorithm': {
            'name': 'SIGVIEW 시즌1 v5.3 - 우상향+압축+CMF매집 (백테스트 검증판)',
            'description': '✅ 백테스트: 평균 60일 +37.6% / 120일 +94.9% / 승률 83% (15종목 × 795회)',
            'main_pattern': 'D = 우상향(60/120선+5%/runup40%+) + 압축(MA10%/BB0.85) + CMF매집',
            'fake_filters': ['ma120_falling', 'dead_cat_bounce', 'volume_excessive', 'cmf_whipsaw'],
        },
        'stocks': results, 'data': data_dict,
        'disclaimer': 'v5.3 = 우상향+압축+CMF매집 (백테스트 평균 60일 +37%/120일 +95%/승률 83%)',
    }
